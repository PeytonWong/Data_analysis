...
for filename in os.listdir('/home/dr/license_data/intersection_right_turn'):
    ...
    print out_str
    bag = rosbag.Bag('/home/dr/license_data/intersection_right_turn/' + filename)
    WriteFilePath = '/home/dr/license_data/data/intersection_right_turn/' + out_str[0] + '_auto_drive.xls'
    ...
    for topic, msg, t in bag.read_messages(topics=['/planner/path']):
        ...
        if path.time_meas < timegnss[0]:
            continue

        if path.time_meas > timegnss[len(timegnss) - 1]:
            break

        curr_time = timegnss[index]
        if path.time_meas > curr_time:
            path_turn_signal.append(path.turn_signal)  # 16
            index += 1
            continue
    ...
    print('path turn signal %f' % len(path_turn_signal))
    # *********************************************************
    # *********************************************************
    timestate0 = []
    ...
    # 5
    # print 'path turn signal'
    data5 = []
    for i in range(0, len(path_turn_signal)):
        # print path_turn_signal[i]
        data5.append(path_turn_signal[i])  # 转向灯状态
    ...
    for i in range(0, len(brake)):  # 制动踏板状态
        ...
        for i in range(0, len(timestate0)):
            ...
            data20.append(0)  # 人工接管提醒　todo 遥控泊车目前没有,暂时赋１,人工接管提醒目前没有可取,暂时赋１,
        ...
        heads = ["原始时间", "时间", "驾驶模式", "数据记录系统状态", "档位", "转向灯状态", "远光灯", "近光灯", "制动踏板状态",
