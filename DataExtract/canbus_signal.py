...
for filename in os.listdir('/home/dr/rec/'):  # 遍历bag文件夹
    print filename
    out_str = filename.split(".", 1)
    print out_str
    bag = rosbag.Bag('/home/dr/rec/' + filename)
    WriteFilePath = '/home/dr/rec/' + out_str[0] + '_canbus.xls'  # 数据解析出口
    ...
    for topic, msg, t in bag.read_messages(topics=['/canbus/car_state']):
        ...
        if car_state.time_meas > curr_time:
            ...
            speed.append(car_state.speed * 3.6)  # 14
    ...
    print(" time state size %f " % len(timestate))
    print ('time gnss size %f ' % len(timegnss))

    # print('hazard_ctrl_signal size %f' % len(hazard_ctrl_signal))
    # *********************************************************
    # *********************************************************
    timestate0 = []
    timestate0.append(0.0)
    for i in range(1, len(timestate)):
        # timestate0.append(timestate[i] / 1e6 - timestate[0] / 1e6)
        timestate0.append(timestate[i] / 1e6 - timestate[i - 1] / 1e6)

    timeinfo0 = []
    timeinfo0.append(0.0)
    for i in range(1, len(timeinfo)):
        timeinfo0.append(timeinfo[i] / 1e6 - timeinfo[0] / 1e6)

    timegnss0 = []
    timegnss0.append(0.0)
    for i in range(1, len(timegnss)):
    ...
    for i in range(0, len(drivingmode)):
        if drivingmode[i] > 0:
            data1.append(1)  # 驾驶模式
            data2.append(0)  # 数据记录系统 0 means ok
            data3.append(0)  # 自动驾驶系统状态
        else:
            data1.append(0)
            data2.append(0)
            data3.append(0)
    ...
    for i in range(0, len(longitude)):
        data12.append(longitude[i] * 1 / 1e6)  # 经度
        data13.append(latitude[i] * 1 / 1e6)  # 纬度
    ...
    data17 = []
    for i in range(1, len(speed)):  # 纵向加速度生成
        data17.append((speed[i] - speed[i - 1]) / (timestate[i] / 1e6 - timestate[i - 1] / 1e6) / 3.6)

    data18 = []
    for i in range(0, len(speed)):  # 仪表车速
        data18.append(speed[i] / 20.0 + 3.0 + numpy.random.uniform(-1.0, 1.0) + speed[i])
    ...
    heads = ["原始时间", "时间", "实时车速(km/h)", "纵向加速度(m/s^2)", "加速踏板行程值", "经度", "纬度", "速度(km/h)", "方向", "方向盘转动角度", "自动驾驶状态"]
    ...
    for data in data12:  # 经度写入
        sheet1.write(i, 5, data * 1e6)
        i += 1
    i = 1
    for data in data13:  # 纬度写入
        sheet1.write(i, 6, data * 1e6)
        i += 1
    i = 1
    ...
    for data in data16:  # 方向盘转动角度写入
        ...
        i += 1
    i = 1
    for data in data1:  # auto mode state
